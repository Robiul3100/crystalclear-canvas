    ...
    >
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-border/50">
        ...
        <div className="flex items-center gap-1">
          <motion.button whileTap={{ scale: 0.9 }} onClick={handleZoomOut} disabled={zoom <= 1}
            className="w-8 h-8 rounded-lg flex items-center justify-center touch-feedback disabled:opacity-30">
            <ZoomOut className="w-4 h-4 text-muted-foreground" />
          </motion.button>
          <span className="text-[12px] font-mono text-muted-foreground w-10 text-center">{zoom.toFixed(1)}×</span>
          <motion.button whileTap={{ scale: 0.9 }} onClick={handleZoomIn} disabled={zoom >= 5}
            className="w-8 h-8 rounded-lg flex items-center justify-center touch-feedback disabled:opacity-30">
          ...
          <AnimatePresence>
            {zoom > 1 && (
              <motion.button initial={{ scale: 0 }} animate={{ scale: 1 }} exit={{ scale: 0 }}
                whileTap={{ scale: 0.9 }} onClick={handleResetZoom}
      ...
      >
        ...
        <div className="absolute inset-0 overflow-hidden" style={{ width: `${position}%` }}>
          <img src={beforeSrc} alt="Before" className="h-full object-contain" draggable={false}
            style={{ width: `${(100 / position) * 100}%`, maxWidth: 'none', transform: `scale(${zoom}) translate(${pan.x / zoom}px, ${pan.y / zoom}px)`, transformOrigin: 'center' }} />
        </div>
        <div className="absolute top-0 bottom-0 w-[2px] bg-white/90 z-10" style={{ left: `${position}%`, transform: 'translateX(-50%)' }}>
          <motion.div whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }}
            className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-10 h-10 rounded-full gradient-primary flex items-center justify-center shadow-lg glow-primary">
      ...
      <AnimatePresence>
        {zoom > 1 && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}
            ...
            <p className="text-[11px] text-muted-foreground">ড্র্যাগ করে প্যান করুন • Shift+ক্লিক করে স্লাইডার সরান</p>
          </motion.div>
        )}
      </AnimatePresence>
    </motion.div>
